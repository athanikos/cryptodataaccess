...
def test_insert_transaction():
    ...
    ut = repo.insert_transaction(1, 1, 'OXT', 1, 1, "USD", "2020-01-01", "kraken",source_id=ObjectId('666f6f2d6261722d71757578'),operation='Added')
    assert (ut.user_id == 1)
...
def test_update_transaction():
    ...
    user_transaction.objects.all().delete()
    ut = repo.insert_transaction(1, 1, 'OXT', 1, 1, "USD", "2020-01-01", "kraken",source_id=ObjectId('666f6f2d6261722d71757578'),operation='Added')
    ut = repo.update_transaction(ut.id, 1, 1, 'OXT2', 1, 1, "EUR", "2020-01-01", "kraken",source_id=ObjectId('666f6f2d6261722d71757578'),operation='Added')
    assert (ut.user_id == 1)
...
def test_delete_transaction_when_exists():
    ...
    repo = TransactionRepository(config, mock_log)
    do_connect(config)
    ut = repo.insert_transaction(1, 1, 'OXT', 1, 1, "EUR", "2020-01-01", "kraken",source_id=ObjectId('666f6f2d6261722d71757578'),operation='Added')
    assert (len(user_transaction.objects) == 1)
